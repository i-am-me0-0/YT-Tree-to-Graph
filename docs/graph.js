  ...
  function render(graph){
    ...
    nodes.forEach(node => {
      if(bonusVideoIds.has(node.id) && node.outgoing) {
        node.outgoing.forEach(out => {
          bonusLinks.push({source: node.id, target: out.to, label: out.label, isBonus: true});
        });
      }
    });
    ...
    const link = container.append('g').attr('class','links')
      ...
      .attr('d', d => {
        ...
        if (bonusNodes.length > 0) { allNodesForLookup.push(...bonusNodes); }
    ...
    const clearButton = d3.select('body').append('button')
      ...
      .text('Clear Lock')
      .on('click', function() {
        isLocked = false; lockedNodeId = null; d3.select(this).style('display', 'none'); node.style('opacity', 1); link.style('opacity', 1); bonusLink.style('opacity', 1); container.selectAll('.arrow-head').style('opacity', 1); node.selectAll('.node-outline').attr('stroke','#333').attr('stroke-width', 3); link.attr('stroke','#666').attr('stroke-width', 2); bonusLink.attr('stroke','#666').attr('stroke-width', 2); container.selectAll('.arrow-head').attr('fill','#666');
      });
    ...
    node.on('mouseover', function(event, d) {
      ...
      node.each(function(nodeData) {
        ...
        const isCurrent = nodeData.data.id === nodeId;
          if(isCurrent) { d3.select(this).style('opacity', 1); } else if(isOutgoing) { d3.select(this).style('opacity', 1); d3.select(this).select('.node-outline').attr('stroke', '#9C27B0').attr('stroke-width', 6); } else if(isDirectIncoming) { d3.select(this).style('opacity', 1); d3.select(this).select('.node-outline').attr('stroke', '#FFC107').attr('stroke-width', 6); } else if(isIndirectIncoming) { d3.select(this).style('opacity', 0.8); d3.select(this).select('.node-outline').attr('stroke', '#00BCD4').attr('stroke-width', 6); } else { d3.select(this).style('opacity', 0.2); }
      });
      ...
      bonusLink.each(function(linkData) { const isOutgoing = linkData.source === nodeId; const isIncoming = linkData.target === nodeId; if(isOutgoing) { d3.select(this).style('opacity', 1).attr('stroke','#9C27B0').attr('stroke-width', 4); } else if(isIncoming) { d3.select(this).style('opacity', 0.7).attr('stroke','#00BCD4').attr('stroke-width', 4); } else { d3.select(this).style('opacity', 0.2); } });
    ...
    }).on('mouseout', function() { if(isLocked) return; node.style('opacity', 1); link.style('opacity', 1); bonusLink.style('opacity', 1); container.selectAll('.arrow-head').style('opacity', 1); node.selectAll('.node-outline').attr('stroke','#333').attr('stroke-width', 3); link.attr('stroke','#666').attr('stroke-width', 2); bonusLink.attr('stroke','#666').attr('stroke-width', 2); container.selectAll('.arrow-head').attr('fill','#666'); }).on('click', function(event, d) { event.stopPropagation(); isLocked = true; lockedNodeId = d.data.id; d3.select('#clear-lock-btn').style('display', 'block'); const mouseoverEvent = new MouseEvent('mouseover'); this.dispatchEvent(mouseoverEvent); });
    ...
    link.on('mouseover', function(event, d) {
      ...
      isLocked = true; lockedNodeId = null; d3.select('#clear-lock-btn').style('display', 'block'); const mouseoverEvent = new MouseEvent('mouseover'); this.dispatchEvent(mouseoverEvent);
    });
    ...
    bonusLink.on('mouseover', function(event, d) {
      ...
      d3.select(this).attr('stroke','#666').attr('stroke-width', 2);
      node.selectAll('.node-outline').attr('stroke','#333').attr('stroke-width', 3);
    }).on('click', function(event, d) { event.stopPropagation(); isLocked = true; lockedNodeId = null; d3.select('#clear-lock-btn').style('display', 'block'); const mouseoverEvent = new MouseEvent('mouseover'); this.dispatchEvent(mouseoverEvent); });
